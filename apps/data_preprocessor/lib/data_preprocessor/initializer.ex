defmodule DataPreprocessor.Initializer do
  ...
  defp import_campaign_data(country, season, league) do
    df = Explorer.DataFrame.from_csv!(Application.app_dir(:data_preprocessor, "priv/data/#{season}/#{league}.csv"))
  ...
  defp persist_team(country, team_name) do
    ...
    |> case do
      {:ok, team_record} -> Logger.info("Successfully added #{team_name} from #{country} to the database, id: #{team_record.id}")
      {:error, changeset} -> Logger.error("An error occurred when inserting #{team_name} : #{inspect changeset}")
    end
  end
  ...
  defp persist_fixture(season, row) do
    home_team = DataPreprocessor.Team |> DataPreprocessor.Repo.get_by(name: Map.get(row, Enum.at(columns(), 2)))
    away_team = DataPreprocessor.Team |> DataPreprocessor.Repo.get_by(name: Map.get(row, Enum.at(columns(), 3)))
    date = Map.get(row, Enum.at(columns(), 1))
    |> String.split("/")
    |> List.to_tuple()
    |> format_date_tuple()
    |> Timex.to_date()
    ...
    |> DataPreprocessor.Repo.insert(on_conflict: :nothing, conflict_target: [:home_team_id, :away_team_id, :season, :date])
    |> case do
      {:ok, fixture_record} -> Logger.info("Successfully added fixture #{fixture_record.id} to the database.")
      {:error, changeset} -> Logger.error("An error occurred: #{inspect changeset}")
    end
  end
  ...
  defp columns do
    ["Div", "Date", "HomeTeam", "AwayTeam", "FTHG", "FTAG", "FTR", "HTHG", "HTAG", "HTR", "B365H", "B365D", "B365A", "BWH", "BWD", "BWA", "IWH", "IWD", "IWA", "LBH", "LBD", "LBA", "PSH", "PSD", "PSA", "WHH", "WHD", "WHA", "SJH", "SJD", "SJA", "VCH", "VCD", "VCA", "Bb1X2", "BbMxH", "BbAvH", "BbMxD", "BbAvD", "BbMxA", "BbAvA", "BbOU", "BbMx>2.5", "BbAv>2.5", "BbMx<2.5", "BbAv<2.5", "BbAH", "BbAHh", "BbMxAHH", "BbAvAHH", "BbMxAHA", "BbAvAHA", "PSCH", "PSCD", "PSCA"]
  end
  ...
  defp format_date_tuple(date_tuple) do
    ...
    year = if String.length(year) == 2 do
      "20#{year}"
    else
      year
